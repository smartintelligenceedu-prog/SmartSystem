...
create table users (
  ...
  locale text not null default 'en' check (locale in ('zh', 'en')), -- migration 039 — persisted UI language preference, synced to a `locale` cookie at login; migration 058 flipped the system default from zh to en
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
