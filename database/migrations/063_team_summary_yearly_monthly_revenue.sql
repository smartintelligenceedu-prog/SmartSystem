-- Adds yearly_revenue/monthly_revenue to team_summary() so the Leader
-- dashboard can show "Overall Team Sales" (existing all-time total_revenue,
-- unchanged), "Yearly Team Sales" (current calendar year), and "Monthly
-- Team Sales" (current calendar month) side by side, instead of only the
-- all-time figure.
-- Adding columns to a RETURNS TABLE function changes its return type, which
-- `create or replace` rejects outright ("cannot change return type of
-- existing function") — drop first, matching migration 007's precedent.
