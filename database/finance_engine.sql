...
-- double-click that recorded the same deposit two or three times) by
-- posting the exact reverse of handle_payment_recorded()'s 'deposit' branch
-- (debit Deposits / credit Cash), then marking the payment 'voided' —
-- record kept, not deleted. Scoped to payment_type='deposit' only; a
-- full_payment/final_payment also flips the invoice/order to 'paid' and
-- recognizes revenue, so undoing those isn't a single reversing entry.
-- ----------------------------------------------------------------------------
...
--
-- Migration 054 — fixes a chaining bug: the reversal entry was inserted
-- with source_type = 'manual_expense' too, so the Finance page's "show a
-- Void button" check couldn't tell it apart from a real expense — voiding
-- an expense produced another voidable row, on and on, each one adding
-- another "作废开销 - " prefix to the description. The reversal now gets its
-- own source_type ('manual_expense_void', which the UI check naturally
-- excludes) and source_id pointing at the entry it reverses, plus an
-- explicit guard against voiding a reversal directly.
-- ----------------------------------------------------------------------------
